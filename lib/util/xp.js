...
function getSite({ build = false } = {}) {
  const sourcePath = build ? paths.BUILD_SITE_DIR : paths.SITE_DIR;
  const fallbackPath = build ? paths.SITE_DIR : paths.BUILD_SITE_DIR;
  const filename = findExistingFilename(sourcePath, ["site.xml", "site.yaml", "cms.yaml"])
    || findExistingFilename(fallbackPath, ["site.xml", "site.yaml", "cms.yaml"]);
