...
async function initialBuild(options = {}) {
  ...
  // Clean up any remaining .xml.jsx files from build output
  // (component files that export functions, not XML strings, aren't deleted during compileFile)
...
async function transformFile(filePath) {
  ...
  // Clean up the transpiled .xml.jsx file from build if it wasn't already removed by compileFile
  ...
  try {
    ...
    // Already deleted by compileFile, ignore
  }
}
 ...
 * These are intermediate transpiled files that XP would otherwise try to parse as XML descriptors.
 */
async function cleanBuildArtifacts() {
  const buildGlob = resolve(util.CODE_DIR, `build/resources/main/**/*${FILE_EXTENSION}`);
  const leftoverFiles = glob.sync(buildGlob);
