...
import { FILE_EXTENSION, XML_RENDER_OPTIONS } from "./constants.js";
...
async function applyBoilerplate(fileContent, relativePath) {
  ...
  const content = (xptMatch(fileContent, /<jsx .*>([\s\S]*)<\/jsx>/m)[1])
    .replace(/<import .*\/>/gmi, "");
  ...
  return renderTemplate("boilerplate", ".xml.jsx", {
...
async function getDocumentInfoForBoilerplate(fileContent) {
  ...
  const [jsxTag] = xptMatch(fileContent, /<jsx .*>/m);
