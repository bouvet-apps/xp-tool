...
import { renderTemplate, RESOURCE_DIR } from "../util/index.js";
import { FILE_EXTENSION, RUNTIME_PATH } from "./constants.js";

const xptMatch = (str, regexp) => str.match(regexp) || [];
...
async function applyBoilerplate(fileContent, relativePath) {
  ...
  // Strip jsx tag and component imports
  const content = (xptMatch(fileContent, /<jsx\b[^>]*>([\s\S]*)<\/jsx>/m)[1])
    .replace(/<import .*\/>/gmi, "");
  ...
  return renderTemplate("boilerplate", ".jsx", {
...
/**
 * Extract info from compact .jsx file
 * @param {string} fileContent Text content of target file
 */
async function getDocumentInfoForBoilerplate(fileContent) {
  ...
  const [jsxTag] = xptMatch(fileContent, /<jsx\b[^>]*>/m);
  const [, componentName] = xptMatch(jsxTag, /name="(.*?)"/);
  const [, props] = xptMatch(jsxTag, /props={{(.*?)}}/);
  ...
  while ((importStatement = reg.exec(fileContent)) !== null) {
    ...
    const [, depComponentName] = xptMatch(statement, /component="(.*?)"/);
    const [, , depModelName] = xptMatch(statement, /model=("|{)(.*?}?)("|})/);
    const [, depModelExport] = xptMatch(statement, /export="(.*?)"/);
    const [, depPath] = xptMatch(statement, /path="(.*?)"/);
...
function resolveBoilerplateDependency({ componentName, modelName }, path = "", currentComponentPath) {
  ...
  if (isComponent) {
    ...
    const places = [
      ...
      resolve(RESOURCE_DIR, "cms/components", componentPath),
